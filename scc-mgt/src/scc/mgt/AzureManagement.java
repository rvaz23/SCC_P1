...
public class AzureManagement {
	...
	static final String MY_SUFFIX = "52656"; // Add your suffix here
	
	static final String AZURE_COSMOSDB_NAME = "scc2122-p-" + MY_SUFFIX;	// Cosmos DB account name
	...
	static final Region[] REGIONS = new Region[] { Region.US_EAST }; // Define the regions to deploy resources here
	...
	static final String[] AZURE_APP_NAME = Arrays.stream(REGIONS).map(reg -> "rv-proj1" + reg.name() + MY_SUFFIX)
			.toArray(String[]::new);
	...
	public synchronized static void dumpRedisCacheInfo(Map<String, String> props, String propFilename, 
		...
		synchronized (props) {
			...
			props.put("USE_CACHE", String.valueOf(USE_CACHE));
		}
		StringBuffer cmd = new StringBuffer();
		if (appName != null) {
			cmd.append("az functionapp config appsettings set --name ");
			...
			cmd.append(String.valueOf(USE_CACHE));
			cmd.append("\"\n");
		}
		if (functionName != null) {
			cmd.append("az functionapp config appsettings set --name ");
					...
					Thread th = new Thread(() -> {
						try {
							...
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "users", "/id",
									null);
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "channels", "/id",
									null);	
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "messages", "/channelId",
									null);
							createCosmosCollection(cosmosClient, AZURE_COSMOSDB_DATABASE, "garbage", "/id",
									null);					
