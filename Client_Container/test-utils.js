...
function printStatus (requestParams, response, context, ee, next) {
     if( response.statusCode === 500)  {
     	...
     	fs.writeFileSync('errors.txt', response.body);
     }    
    return next();
} 
...
function adddToSubscribed(context, events, done) {
	if( context.vars.user!==undefined) {
		let user = {id:context.vars.user, pwd:context.vars.pwd}
		subscribed.push(user) 
		fs.writeFileSync('subscribed.data', JSON.stringify(subscribed));
	}
	return done()
}

function adddToOwners(context, events, done) {
	if( context.vars.user!==undefined) {
		let user = {id:context.vars.user, pwd:context.vars.pwd}
		owners.push(user) 
		fs.writeFileSync('owners.data', JSON.stringify(owners));
	}
	return done()
}
